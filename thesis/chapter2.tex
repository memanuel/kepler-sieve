...
Indeed, the \tty{astropy} astronomy package has the capability to perform these calculations.
...
\begin{figure}[hbt!]
\begin{center}
\includegraphics[width=0.8\textwidth]{../figs/web/ICRS_USNO.jpg}
...
Rather than trying to reinvent the wheel, I use \tty{astropy} as much as possible in the next section
...
At initialization, an \tty{AsteroidDirection} layer requires an array \tty{ts} of MJDs, the row lengths for each observation in the batch, and the site name.
...
\tty{03\_RA\_DEC.ipynb} in the \tty{jupyter} directory of the \href{https://github.com/memanuel/kepler-sieve}{project repository}.
...
\hline
\end{tabular}
\caption[Mean differences between JPL, SkyField, and my calculations of astrometric direction]
{Mean differences between JPL, SkyField, and my calculations (MSE) of astrometric direction in Arc Seconds\\
...
$H$ is a measure of its absolute brightness, and $G$ is how sensitive its aparent magnitude will be as
...
I was forced to disregard the predicted magnitude durring the fitting process.
A first pass of the code is already present in the \tty{AsteroidModel} class, though,
