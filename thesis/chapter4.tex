...
\begin{figure}[hbt!]
\begin{center}
\includegraphics[width=0.68\textwidth]{../figs/elts/v_hist_unperturbed.png}
\includegraphics[width=0.68\textwidth]{../figs/elts/v_hist_perturbed.png}
\includegraphics[width=0.68\textwidth]{../figs/elts/v_hist_random.png}
\end{center}
...
i.e. think of the PDF $f(s)$ as being normal with PDF $f(s) \propto e^{-s^2 / 2 R^2}$.\\
...
$$ f(v| \mathrm{Hit}, \lambda) = \frac{\lambda e^{-\lambda v}}{1 - e^{-\lambda}}$$
...
&= h \cdot \frac{\lambda e^{-\lambda v}}{1 - e^{-\lambda}} + (1 - h)
\end{align*}

The optimization objective function will be the log-likelihood of the PDF:
$$ \mathcal{L}(\vvec, h, \lambda) = \sum_{j=1}^{n} \log \left( h_j \cdot \frac{\lambda_j e^{-\lambda_j v_j}}{1 - e^{-\lambda_j}} + 1 - h_j \right)$$
...
\footnote{$N_h$ is automatically reduced to be at most $N_{\textrm{close}}$, 
the number of observations that are within the current threshold $\tau$.
Omitting this check can lead to absurd results with $h>1$ when $\tau$ is discontinuously lowered
as part of a set training schedule.}
...
But if you've trained to the point of a resolution of 0.1 degrees, there's no reason to drag around observations that are $20 \times$ further away.
...
we saw that the quantity $T = n^{-3/2} \cdot \sum_{i=1}^{n} 1 - \log V_i \sim \mathcal{N}(0,1)$ had the same distribution regardless of $n$.
