...
\section{The Angular Distance Between two Directions $\vec{u}_1$ and $\vec{u}_2$}
...
Loading one block of 1,000 asteroids therefore has only 3.81 million numbers ($635$ days $\times 1,000$ asteroids $\times 6$ numbers per integration point).
\footnote{An approach with far superior scaling would be to use some form of spatial indexing, 
perhaps a K-D tree or a database back end with support for spatial indices.
For this problem, the modestly optimized brute force approach presented here was sufficient and consumed less development time.}
...
The job ended up being memory bound, maxing out all 256 GB of available RAM on the server.
The initial job described above computes only the nearest asteroid in a block of 1,000 asteroids.
...
We can relate the Cartesian distance $s$ to the height $z$ of our guess with the following simple observations.\\
