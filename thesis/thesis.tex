...
\hypersetup{
   ...
   pdfsubject={Harrvard IACS Masters Thesis (May 2020)},
   pdfkeywords={},
   pdfcreator={pdfLaTeX},
...
Candidate orbital elements are initialized by randomly choosing each of the six elements independently from a known asteroid.
A statistical model describes the distribution of angular distances between the directions from the observatory site 
and the predicted position of a body with these orbital elements.
This model yields a log likelihood function, which attains large positive values when the candidate orbital elements that are near to the elements
of a real asteroid viewed many times in the data.
