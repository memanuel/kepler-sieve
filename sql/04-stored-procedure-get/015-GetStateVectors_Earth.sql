...
COMMENT "Get the state vectors for Earth in a date range. Stride can be a multiple of 5 minutes."
...
CREATE OR REPLACE 
DEFINER = kepler
PROCEDURE KS.GetStateVectors_Earth_Minute(
    IN mjd0 INT,
    IN mjd1 INT,
    IN stride INT
)
COMMENT "Get the state vectors for Earth in a date range. Stride can be a multiple of 1 minute."
